...
--output_dir $OUTPUT_DIR --port $PORT --resume $RESUME  --eval \
"

# local multi/single GPU
# python -m torch.distributed.launch --nproc_per_node=$GPUS --master_port=$PORT \
#     --use_env main.py --config $CONFIG  --data-set IMNET --batch-size $batch_size --output_dir $OUTPUT_DIR --resume $RESUME --eval\
