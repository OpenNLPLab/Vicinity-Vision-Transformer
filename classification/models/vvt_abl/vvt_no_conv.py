...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., 
        ...
        self.mlp = Mlp_original(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim, act_layer=act_layer, drop=drop)
...
class VicinityVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            pos_embed = nn.Parameter(torch.zeros(1, seq_len, embed_dims[i]))
...
### VVT
@register_model
def vvt_no_conv_test(pretrained=False, **kwargs):
...
@register_model
def vvt_no_conv_tiny(pretrained=False, **kwargs):
    model = VicinityVisionTransformer(
...
@register_model
def vvt_no_conv_small(pretrained=False, **kwargs):
    model = VicinityVisionTransformer(
        patch_size=4, embed_dims=[96, 160, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
@register_model
def vvt_no_conv_medium(pretrained=False, **kwargs):
    model = VicinityVisionTransformer(
        patch_size=4, embed_dims=[96, 160, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
@register_model
def vvt_no_conv_large(pretrained=False, **kwargs):
    model = VicinityVisionTransformer(
